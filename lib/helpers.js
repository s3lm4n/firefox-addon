...
const PriceTrackerHelpers = (function () {
  ...
  async function setStorage(key, value) {
    try {
      ...
      // PROTECTION: Special handling for trackedProducts to prevent accidental data loss
      if (key === "trackedProducts") {
        if (!Array.isArray(value)) {
          console.error("[Storage] ❌ Attempted to save non-array as trackedProducts");
          throw new Error("trackedProducts must be an array");
        }

        // Get existing count
        const existing = await browser.storage.local.get("trackedProducts");
        const existingProducts = existing.trackedProducts || [];
        const existingCount = Array.isArray(existingProducts) ? existingProducts.length : 0;

        // Warn if we're about to save significantly fewer products
        if (existingCount > 0 && value.length < existingCount * 0.5) {
          console.warn(`[Storage] ⚠️ WARNING: Saving ${value.length} products when we had ${existingCount}. Creating backup.`);
          // Auto-backup before potentially destructive operation
          await browser.storage.local.set({
            trackedProducts_autoBackup: existingProducts,
            trackedProducts_autoBackup_time: Date.now()
          });
        }

        // Log significant changes
        if (value.length !== existingCount) {
          console.log(`[Storage] 📊 Products: ${existingCount} → ${value.length}`);
        }
      }
