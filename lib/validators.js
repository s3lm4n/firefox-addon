...
const Validators = (function () {
  ...
  function sanitizeProductData(product) {
    if (!product || typeof product !== "object") {
      console.warn("[Validators] Invalid product object:", product);
      return null;
    }
    ...
    if (!isValidPrice(price)) {
      console.warn("[Validators] Invalid price for product:", product.name, "price:", product.price);
      return null;
    }

    // Sanitize name - be more lenient, allow short names
    let name = (product.name || "").toString().trim();
    if (!name || name.length < 1) {
      console.warn("[Validators] Invalid name for product:", product.url);
      return null;
    }

    // Validate URL
    if (!isValidUrl(product.url)) {
      console.warn("[Validators] Invalid URL for product:", product.name, "url:", product.url);
      return null;
    }
    ...
    return {
      ...
      domain: product.domain || extractDomain(product.url),
      ...
      // Custom selector support - CRITICAL: preserve these!
      customSelector: product.customSelector || null,
      selectorPath: product.selectorPath || null,
    };
  }
