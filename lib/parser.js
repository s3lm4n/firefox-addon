...
const PriceParser = {
  ...
  // Metinden fiyat parse et (gelişmiş regex)
  parsePrice(text) {
    if (!text) return null;
    ...
    const patterns = [
      // TL patterns
      { regex: /(\d{1,3}(?:\.\d{3})+,\d{2})\s*(?:TL|₺|tl)/i, type: 'tr', currency: 'TL', hasCurrency: true },
      { regex: /(\d+,\d{2})\s*(?:TL|₺|tl)/i, type: 'tr', currency: 'TL', hasCurrency: true },
      
      // USD patterns
      { regex: /\$\s*(\d{1,3}(?:,\d{3})*(?:\.\d{2})?)/i, type: 'us', currency: '$', hasCurrency: true },
      { regex: /(\d{1,3}(?:,\d{3})*\.\d{2})\s*USD/i, type: 'us', currency: '$', hasCurrency: true },
      
      // EUR patterns
      { regex: /€\s*(\d{1,3}(?:[,\.]\d{3})*[,\.]\d{2})/i, type: 'eu', currency: '€', hasCurrency: true },
      { regex: /(\d{1,3}(?:[,\.]\d{3})*[,\.]\d{2})\s*EUR/i, type: 'eu', currency: '€', hasCurrency: true },
      
      // GBP patterns
      { regex: /£\s*(\d{1,3}(?:,\d{3})*\.\d{2})/i, type: 'gb', currency: '£', hasCurrency: true },
      
      // Generic patterns (para birimi yok)
      { regex: /(\d{1,3}(?:\.\d{3})+,\d{2})/i, type: 'tr', currency: null, hasCurrency: false },
      { regex: /(\d{1,3}(?:,\d{3})+\.\d{2})/i, type: 'us', currency: null, hasCurrency: false },
      { regex: /(\d+[,\.]\d{2})/i, type: 'generic', currency: null, hasCurrency: false }
    ];
